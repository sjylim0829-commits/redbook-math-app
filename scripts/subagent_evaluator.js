...
async function runSubagentEvaluation() {
  ...
  try {
    ...
    const interactiveUsesLerp = htmlContent.includes("startSmoothLerp('absPos'");
    const ok = hasLerp && htmlHasLerp && interactiveUsesLerp;
    recordCheck('INTENT-16', '절전형 물리 애니메이션 엔진 (startSmoothLerp) 및 시뮬레이터 연동', ok,
      ok ? '지수 감속(0.12), rAF 절전 종료, 2단원 수직선 절댓값 시뮬레이터 실시간 Lerp 연동 확인' : 'startSmoothLerp 미탑재 또는 시뮬레이터 미연동');
  } catch (e) {
