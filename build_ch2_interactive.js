  ...
  canvasDrawersJs: `
    ...
    function setAbsPos(val) {
      ...
      if (typeof window.startSmoothLerp === 'function') {
        window.startSmoothLerp('absPos', () => simState.p14Pos, (v) => {
          simState.p14Pos = Math.round(v);
          const readout = document.getElementById('abs-readout');
          if (readout) readout.innerText = '|' + simState.p14Pos + '| = ' + Math.abs(simState.p14Pos);
          const two = twoInstance;
          if (two) renderAbsCanvas(two, simState.p14Pos);
        }, target);
      } else {
        simState.p14Pos = val;
        const readout = document.getElementById('abs-readout');
